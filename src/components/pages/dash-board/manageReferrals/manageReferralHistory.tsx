...
const ManageReferralHistory = ({ visibleFilter }: { visibleFilter: Record<string, boolean> }) => {
      ...
      <Table className="border-collapse">
        <TableHeader className="rounded-lg h-[60px] [&_tr]:border-b">
          <TableRow className="bg-[#ffffff] hover:bg-white border-b font-bold leading-[150%] text-[14px] text-[#121826]">
            ...
            {visibleFilter.transactionAmount && <TableHead className="text-center font-bold text-[#121826] border-r border-gray-300">Transaction Amount</TableHead>}
        ...
        <TableBody>
          {transaction.map((transaction, index) => (
            <TableRow key={index} className="cursor-pointer odd:bg-gray-100 even:bg-gray-200 border-b h-[50px] hover:bg-gray-300 transition-all">
              {visibleFilter.user && (
              ...
              {visibleFilter.transactionAmount && <TableCell className="py-2 text-center border-r border-gray-300">{transaction.transactionAmount}</TableCell>}
              {visibleFilter.Status && (
                <TableCell className="py-2 text-center border-r border-gray-300">
                  <span
                    className={`px-2 py-1 rounded-sm text-xs font-medium ${
                      transaction.Status === "Verified" ? "text-[#121826]" : transaction.Status === "UnVerified" ? "text-red-700" : "text-yellow-700"
                    }`}
                  >
                    {transaction.Status}
                  </span>
              ...
              {visibleFilter.action && (
                <TableCell className="text-center py-2">
                  <span
                    className={`px-2 py-1 rounded-sm text-xs font-medium ${
                      transaction.action === "Finish" ? "text-[#0073AD]" : transaction.action === "Unverified" ? "text-red-700" : "text-yellow-700"
                    }`}
                    onClick={() => setShowOTP(true)}     // Trigger OTP when "Finish" is clicked
                  >
                    {transaction.action}
                  </span>
