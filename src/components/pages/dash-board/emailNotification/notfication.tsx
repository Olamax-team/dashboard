    ...
    Today: [
      {
        ...
        message: "Your crypto has been converted to Naira. Check your wallet for the updated balance.",
        time: "1h",
        type: "success",
      },
      {
        ...
        message: "We've added a quick-buy option for USDT. Access faster transactions directly from your dashboard.",
        time: "1h",
        type: "info",
      },
    ],
    Yesterday: [
      {
        ...
        message: "Please verify your email address and enable two-factor authentication to keep your account secure.",
        time: "1h",
        type: "security",
      },
      {
        ...
        message: "Only a few days left to earn extra BONK Tokens on every BTC purchase. Don't miss out!",
        time: "1h",
        type: "reward",
      },
      {
  ...
  const messages: Message[] = [
    {
      ...
      messageIfo: "We've added a quick-buy option for USDT. Access faster transactions directly from your dashboard.",
      time: "1h",
    },
    {
      avatar: avatar,
    ...
    <div className="fixed inset-0 flex items-end justify-end bg-black/50" onClick={() => setEmailIsOpen(false)}>
      <div className="w-[350px] h-screen lg:w-[500px] bg-white rounded-lg shadow-lg pt-10 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <button onClick={() => setEmailIsOpen(false)} className="text-gray-500 hover:text-gray-700">
            <X className="h-6 w-6" />
          </button>
        </div>
                    ...
                    <div key={notification.id} className="flex gap-3 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between">
                          <p className="text-[18px] font-bold text-[#181818] leading-[150%]">{notification.title}</p>
                          <span className="text-[12px] font-semibold text-gray-500">{notification.time}</span>
                        </div>
                        <p className="text-[12px]  font-medium leading-[160%] text-gray-600 mt-1">{notification.message}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {messages.map((msg, index) => (
                ...
                <div key={index} className="flex gap-5 py-3 border-b pb-3" onClick={() => setActiveTab("chatBox")}>
                  ...
                  <div className="flex-1 min-w-0" onClick={() => setActiveTab("chatBox")}>
                    ...
                    <p className="text-[12px] font-medium   leading-[160%] text-gray-600 mt-1">{msg.messageIfo}</p>
                  </div>
                </div>
              ))}
            </>
