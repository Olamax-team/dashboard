...
const ChartContent = () => {
  ...
  const totalSell = chartData.reduce((sum, item) => sum + Math.abs(item.sell), 0)
      ...
      <CardContent className="">
        <div className="flex items-center justify-between ">
            ...
            <ChartContainer config={chartConfig}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                  ...
                  />
                   <YAxis
                    ...
                    tick={({ x, y, payload }) => (
                      ...
                      >
                        {payload.value === 0 ? "0" : `${Math.abs(payload.value / 1000)}k`}
                      </text>
                    )}
                  />
                  ...
                  <Bar dataKey="sell" fill={chartConfig.sell.color} radius={4} />
                </BarChart>
              </ResponsiveContainer>
            </ChartContainer>
          </div>
          ...
          <div className="mb-10 flex flex-col justify-center space-y-10 ">
            <div>
              <div className="text-[#121826]  font-normal  text-[10px] lg:text-[15px] mb-1">Fiscal Year Buy</div>
              <p className="lg:text-[20px] text-[15px] font-medium text-[#121826]">{formatCurrency(totalBuy)}</p>
            </div>
            <div>
              <div className="text-[#121826] text-[10px] lg:text-[16px] mb-1">Fiscal Year Sell</div>
              <p className="lg:text-[20px] text-[15px] font-medium text-[#121826]">{formatCurrency(totalSell)}</p>
            </div>
          </div>
        </div>
      </CardContent>
