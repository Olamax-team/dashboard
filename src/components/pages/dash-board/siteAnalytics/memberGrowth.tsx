...
const MemberGrowthChart = () =>{
  const memberData = [240000, 140000, 330000, 140000, 220000, 460000, 160000, 480000, 280000, 420000]
      ...
      y: {
        ...
        ticks: {
          ...
          callback: (value: string | number) =>
            typeof value === "number" ? (value === 0 ? "0" : `${value / 1000}k`) : value,
                    font: {
            size: 15,
          },
        },
  ...
  const refreshPrices = useCallback(() => {
    setCryptoData((prevData) =>
      prevData.map((crypto) => {
        ...
        const formattedPrice = `NGN${newPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`
  ...
  return (
    <div className="lg:flex  justify-between    ">
        <div className=" lg:w-[60%] w-full rounded-lg  bg-white  ">
     ...
     <div className="lg:w-[40%]  w-full h-auto p-5 bg-white rounded-lg ">
      <div className="pb-3 flex flex-row items-center justify-between">
        ...
        <button onClick={refreshPrices} className="p-1 rounded-full hover:bg-slate-100" aria-label="Refresh prices">
          <RefreshCw size={16} />
        </button>
      </div>
      <div className="pt-3">
          ...
          {cryptoData.map((crypto, index) => (
            <div key={index} className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="   pt-2">
                  <div className="font-medium text-[14px] text-[#121826]">{crypto.symbol}</div>
                  <div className="text-xs text-muted-foreground">{crypto.name}</div>
                </div>
              </div>
              <div className="text-right  pt-2">
                <div className="font-medium text-[14px]  text-[#121826]">{crypto.price}</div>
                <div
                  className={`text-xs flex items-center justify-end ${crypto.change >= 0 ? "text-[#1FAF38]" : "text-[#E41D03]"}`}
                >
